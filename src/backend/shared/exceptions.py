...
class BisonCoinException(Exception):
    ...
    def __init__(
        self,
        json_message=FailureReturnString.UNKNOWN_FAILURE.value,
        return_code=HttpCode.INTERNAL_SERVER_ERROR.value,
    ):
...
class TransactionVerificationException(BisonCoinException):
    def __init__(
        self,
        json_message=FailureReturnString.SIGNATURE_VERFICATION_FAILURE.value,
        return_code=HttpCode.BAD_REQUEST.value,
    ):
...
class ReceiverException(BisonCoinException):
    def __init__(self):
        super().__init__(
            FailureReturnString.RECEIVER_NOT_PRESENT.value, HttpCode.BAD_REQUEST.value
        )
...
class DatabaseVerificationException(BisonCoinException):
    def __init__(self, error_messsage="Unknown Error"):
        ...
        super().__init__(
            FailureReturnString.DATABASE_VERIFICATION_FAILURE.value,
...
class IncorrectCredentialsException(BisonCoinException):
    def __init__(self):
        super().__init__(
            FailureReturnString.INCORRECT_CREDENTIALS.value, HttpCode.BAD_REQUEST.value
        )
