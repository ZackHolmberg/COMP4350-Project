...
def test_create_transaction_wrong_wallet_amount(
    test_client, json_header, requests_mock, public_key, signature
):
    ...
    requests_mock.post(
        "http://api-gateway/blockchain/wallet/createTransaction",
        json={"valid": False},
        status_code=400,
    )

    requests_mock.post(
        "http://api-gateway/blockchain/wallet/checkWallet",
        json={"valid": True},
        status_code=200,
    )

    requests_mock.get(
        "http://users:5000/umnetId/USER1",
...
def test_create_transaction_correct_payload(
    test_client, json_header, requests_mock, signature, public_key
):
    ...
    requests_mock.post(
        "http://mining:5000/queue", json={"success": True}, status_code=201
    )
...
def test_create_transaction_receiver_verification_failure(
    test_client, json_header, requests_mock, signature, public_key
):
