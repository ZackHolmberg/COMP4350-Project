...
with patch.object(
    database_init, "create_users_db", return_value=mongomock.MongoClient()
):
...
user_one = {
    ...
    "last_name": "Sharma",
    "umnetId": "SHARMAA2",
    "password": generate_password_hash(user_one_pass, method="sha256"),
...
user_two = {
    ...
    "last_name": "Sachdev",
    "umnetId": "SACHDEV1",
    "password": generate_password_hash(user_two_pass, method="sha256"),
    "public_key": "abhi_pk",
}
...
def requests_mock_users(requests_mock):
    requests_mock.post(
        "http://api-gateway/blockchain/wallet/addWallet",
        json={"success": True},
        status_code=200,
    )
...
def test_create_user(test_client, json_header, requests_mock_users, test_payload):
    ...
    response = test_client.post(url, data=json.dumps(test_payload), headers=json_header)
...
def test_create_error_duplicate_umnetId_and_password(
    test_client, test_db_patch, test_payload, json_header
):
    ...
    assert (
        FailureReturnString.DATABASE_VERIFICATION_FAILURE.value.encode()
        in response.data
    )
...
def test_create_error_incomplete_payload(
    test_client, test_db_patch, json_header, test_payload
):
    url = "/create"
...
def test_success_update_user(
    test_client, test_db_patch, json_header, update_user_payload
):
    url = "/update"
...
def test_failure_update_user_incomplete_payload(
    test_client, test_db_patch, json_header, test_payload
):
    url = "/update"
