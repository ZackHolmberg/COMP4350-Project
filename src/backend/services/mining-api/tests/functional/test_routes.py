...
import pytest
from src import app, routes, socketio
from src.routes import difficulty, send_to_connected_clients
...
def test_proof_correct_payload(test_client, json_header, requests_mock):
    ...
    requests_mock.post(
        "http://api-gateway/blockchain/addBlock",
        json={"success": True},
        status_code=201,
    )
    ...
    socketio_test_client.emit(
        "proof",
        {"id": VALID_ID, "proof": hash_, "nonce": VALID_NONCE, "minerId": "test"},
    )
...
def test_proof_incorrect_id(test_client, json_header, requests_mock):
    ...
    hash_ = sha256(
        (str(5) + "test1" + "test2" + str(10) + "test3" + "test4").encode("utf-8")
    ).hexdigest()
    socketio_test_client.emit(
        "proof", {"id": "test2", "proof": hash_, "nonce": 5, "minerId": "test"}
    )
...
def test_proof_incorrect_hash(test_client, json_header, requests_mock):
    ...
    hash_ = sha256(
        (str(5) + "test2" + "test2" + str(10) + "test3" + "test4").encode("utf-8")
    ).hexdigest()
    socketio_test_client.emit(
        "proof", {"id": "test3", "proof": hash_, "nonce": 5, "minerId": "test"}
    )
