import pytest
from src import app
import json
import sys
import os
sys.path.append(os.path.abspath(os.path.join('../..', '')))

from shared import HttpCode
from shared.utils import BisonCoinUrls

balance_url = BisonCoinUrls.blockchain_wallet_url.format("balance")
auth_url = BisonCoinUrls.user_api_url.format("authUser")
chain_url = BisonCoinUrls.blockchain_url.format("chain")


@pytest.fixture(scope='module')
def test_client():
    test_client = app.test_client()

    ctx = app.app_context()
    ctx.push()

    yield test_client

    ctx.pop()

@pytest.fixture(scope='module')
def test_chain():
    chain =  [
        "{\"hash\": \"0000\", \"index\": 0, \"miner_id\": \"miner_id\", \"nonce\": 0, \"prev_hash\": \"0\", \"reward_amount\": 0, \"transaction\": {\"amount\": 0, \"from_address\": \"\", \"id\": \"\", \"signature\": \"\", \"timestamp\": 0, \"to_address\": \"\"}}",
        "{\"hash\": \"0000813999da5fdc0a61293c8b38a91a0b2a1864c3f7b431ed9684b04c53da45\", \"index\": 1, \"miner_id\": \"SACHDEV1\", \"nonce\": 85269, \"prev_hash\": \"0000\", \"reward_amount\": 10, \"transaction\": {\"amount\": 2, \"from_address\": \"SACHDEV1\", \"id\": \"012d92c16a0841bd4dfd7f8ad071a284e9772ed6f0c755fed4de894f6b0cebf5\", \"signature\": \"\", \"timestamp\": 1616457752, \"to_address\": \"SHARMAA2\"}}",
        "{\"hash\": \"00003ff58178aa6383207fc8980530066e2ff3d8e5e91cbebd639615c3561a9d\", \"index\": 2, \"miner_id\": \"SACHDEV1\", \"nonce\": 15368, \"prev_hash\": \"0000813999da5fdc0a61293c8b38a91a0b2a1864c3f7b431ed9684b04c53da45\", \"reward_amount\": 10, \"transaction\": {\"amount\": 12, \"from_address\": \"SACHDEV1\", \"id\": \"64713990ecb099eaeac7ecaf9fa7559b3fc35c3b7c6008a2328591906b52606d\", \"signature\": \"\", \"timestamp\": 1616457767, \"to_address\": \"SHARMAA2\"}}",
        "{\"hash\": \"000095c474acef6e2bfa143a178a0515969775ca597c04a03752049c21da8b09\", \"index\": 3, \"miner_id\": \"SHARMAA2\", \"nonce\": 16310, \"prev_hash\": \"00003ff58178aa6383207fc8980530066e2ff3d8e5e91cbebd639615c3561a9d\", \"reward_amount\": 10, \"transaction\": {\"amount\": 3, \"from_address\": \"SACHDEV1\", \"id\": \"bf3ca14719becea7a176e0b295d4b6bb9ce1e11461968ff435afe7753d4a4e5c\", \"signature\": \"\", \"timestamp\": 1616457911, \"to_address\": \"SHARMAA2\"}}"
    ]
    return chain

@pytest.fixture(scope='function')
def requests_mock_wallet(requests_mock, test_chain):
    requests_mock.get(balance_url, json={"amount": 0})
    requests_mock.post(auth_url, json={"success": True})
    requests_mock.get(chain_url, json={ "chain": test_chain, "length": len(test_chain)}, status_code=200)
    
    return requests_mock


def test_home_page(test_client):
    # test GET query on '/' route
    url = '/'
    response = test_client.get(url)

    assert response.status_code == HttpCode.OK.value
    assert b"Hello from your wallet" in response.data

def test_get_wallet_amount_success(test_client, requests_mock_wallet):
    url = '/amount'

    response = test_client.post(url, json={"umnetId": 'to_be_genetated_elsewhere', "password" : ""})

    assert response.status_code == HttpCode.OK.value
    assert json.loads(response.data)["amount"] == 0


def test_get_wallet_amount_error(test_client, requests_mock_wallet):
    url = '/amount'

    requests_mock_wallet.get(balance_url, json={"error": "no corresponding wallet for id"}, status_code=400)
    response = test_client.post(url, json={"umnetId": 'to_be_genetated_elsewhere', "password" : ""})

    assert response.status_code == HttpCode.BAD_REQUEST.value
    assert json.loads(response.data)["error"] == "no corresponding wallet for id"


def test_get_wallet_amount_incorrect_payload(test_client, requests_mock_wallet):
    url = '/amount'
    response = test_client.post(url, json={})

    assert response.status_code == HttpCode.BAD_REQUEST.value
    assert json.loads(response.data)["error"] == "Please send correct json payload"

def test_transaction_history_successfull_response(test_client, requests_mock_wallet):
    url = '/history/SHARMAA2'
    response = test_client.get(url)

    assert response.status_code == HttpCode.OK.value
    assert b'"history":[{"transaction":{"amount":3,"from_address":"SACHDEV1","id":"bf3ca14719becea7a176e0b295d4b6bb9ce1e11461968ff435afe7753d4a4e5c","signature":"","timestamp":1616457911,"to_address":"SHARMAA2"},"type":"receive"},{"transaction":{"amount":10,"from_address":"BLOCKCHAIN","id":"bf3ca14719becea7a176e0b295d4b6bb9ce1e11461968ff435afe7753d4a4e5c","signature":"","timestamp":1616457911,"to_adderss":"SHARMAA2"},"type":"reward"}' in response.data

def test_transaction_history_success_empty_response(test_client, requests_mock_wallet):
    url = '/history/DoesntExist'
    response = test_client.get(url)

    assert response.status_code == HttpCode.OK.value
    assert response.get_json()["history"] == []
    
def test_get_wallet_amount_auth_failure(test_client, requests_mock_wallet):
    url = '/amount'
    requests_mock_wallet.post(auth_url, json={"success": False})

    response = test_client.post(url, json={"umnetId" : "um", "password": "wrong_password"})
    assert not response.json['success']

def test_get_wallet_amount_auth_exception(test_client, requests_mock_wallet):
    url = '/amount'
    requests_mock_wallet.post(auth_url, json={"error": "AUTH FAILURE"})

    response = test_client.post(url, json={"umnetId" : "um", "password": "wrong_password"})
    assert b'AUTH FAILURE' in response.data
